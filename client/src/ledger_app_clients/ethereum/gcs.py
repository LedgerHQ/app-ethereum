...
class PathArray:
    ...
    def serialize(self) -> bytes:
        ...
        if self.length is not None:
            payload += format_tlv(0x03, self.length)
        return payload
    ...
    def serialize(self) -> bytes:
        ...
        for node in self.path:
            if isinstance(node, PathTuple):
                ...
                assert False, f"Unkown path node type : {type(node)}"
...
ParamUnion = ParamRaw | ParamAmount | ParamTokenAmount | ParamNFT | ParamDatetime | ParamDuration | ParamUnit | ParamTrustedName | ParamEnum
