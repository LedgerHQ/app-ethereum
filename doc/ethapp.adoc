...
The filter flags are simple booleans which indicate if the filter should be expected or not.
A callee filter flag set to `false` means the verifyingContract from the EIP712Domain should be used as the callee address.

The calldata index is there to differentiate calldatas the message contains multiple and if they are presented in a way where the filters would be intertwined (the app would start receiving filters about a second calldata before receiving all the filter from the first one).
...
| Calldata index        | 1
| Value filter flag     | 1
| Callee filter flag    | 1
| Chain ID filter flag  | 1
| Selector filter flag  | 1
| Amount filter flag    | 1
| Spender filter flag   | 1
| Signature length      | 1
| Signature             | variable
|==========================================
