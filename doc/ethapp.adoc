...
For each intermediate APDU, the Status Word will 0x61XX, giving the remaining available entries in the lower bits.
For example, it will return `0x6102` when the command is correctly completed, and there are still `2` remaining (free) entries.
