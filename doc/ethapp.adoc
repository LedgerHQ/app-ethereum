...
Up to *2* different configurations can be used.The targeted slot is configured using P2.
...
> Note For the icon:
>
>  - It is not supported on Nano devices and the corresponding APDU will be ignored.
>
>  - Depending on its size, it may be split on several chunks. The maximum icon buffer size allowed is `1KB`.
>
>  - Its value is the hex string generated by the script `<SDK_PATH>/lib_nbgl/tools/icon2glyph.py`, with parameter `--hexbitmap`
...
|==============================================================
| *CLA*     | *INS*    | *P1*                       | *P2*                     | *LC*     | *Le*
.3+| E0  .3+| 30       | 00 : Network configuration | <slot>                   | variable | variable
                       | 01 : Network Icon          | MSB:<slot>, LSB 01 : First chunk
