...
#define P1_NETWORK_CONFIG 0x00
#define P1_NETWORK_ICON   0x01
#define P1_GET_INFO       0x02
#define P2_FIRST_CHUNK    0x01
#define P2_NEXT_CHUNK     0x00
 ...
 * @param[in] data buffer received
 * @param[in] field_len Length of the field value
 * @param[in] slot Network slot index
 * @return APDU Response code
 */
static uint16_t parse_chain_id(const uint8_t *data, uint16_t field_len, uint8_t slot) {
    uint64_t chain_id;
    uint64_t max_range;
    uint8_t i;
    ...
    DYNAMIC_NETWORK_INFO[slot].chain_id = chain_id;
    return APDU_RESPONSE_OK;
}
 ...
 * @return APDU Response code
 */
static uint16_t parse_name(const uint8_t *data, uint16_t field_len, uint8_t slot) {
    CHECK_FIELD_OVERFLOW("NETWORK_NAME", DYNAMIC_NETWORK_INFO[slot].name);
    // Check if the name is printable
    if (!check_name(data, field_len)) {
        PRINTF("NETWORK_NAME is not printable!\n");
        return APDU_RESPONSE_INVALID_DATA;
    }
    COPY_FIELD(DYNAMIC_NETWORK_INFO[slot].name);
    return APDU_RESPONSE_OK;
}
 ...
 * @return APDU Response code
 */
static uint16_t parse_ticker(const uint8_t *data, uint16_t field_len, uint8_t slot) {
    CHECK_FIELD_OVERFLOW("NETWORK_TICKER", DYNAMIC_NETWORK_INFO[slot].ticker);
    // Check if the ticker is printable
    if (!check_name(data, field_len)) {
        PRINTF("NETWORK_TICKER is not printable!\n");
        return APDU_RESPONSE_INVALID_DATA;
    }
    COPY_FIELD(DYNAMIC_NETWORK_INFO[slot].ticker);
    return APDU_RESPONSE_OK;
}
 ...
 * @return APDU Response code
 */
static uint16_t parse_icon_hash(const uint8_t *data, uint16_t field_len, uint8_t slot) {
    CHECK_FIELD_LENGTH("NETWORK_ICON_HASH", field_len, CX_SHA256_SIZE);
    COPY_FIELD(g_network_icon[slot].hash);
    return APDU_RESPONSE_OK;
}
...
static void print_icon_info(void) {
    ...
    PRINTF("[NETWORK_ICON] - Registered in slot %d: icon %dx%d (BPP %d)\n",
           g_icon_payload.slot,
           DYNAMIC_NETWORK_INFO[g_icon_payload.slot].icon.width,
           DYNAMIC_NETWORK_INFO[g_icon_payload.slot].icon.height,
           DYNAMIC_NETWORK_INFO[g_icon_payload.slot].icon.bpp);
}
...
static uint16_t parse_icon_buffer(void) {
    ...
    const uint8_t *data = g_network_icon[g_icon_payload.slot].bitmap;
    ...
    CHECK_FIELD_OVERFLOW("NETWORK_ICON", g_network_icon[g_icon_payload.slot].bitmap);

    // Check icon hash
    cx_sha256_hash(data, field_len, digest);
    if (memcmp(digest, g_network_icon[g_icon_payload.slot].hash, CX_SHA256_SIZE) != 0) {
        PRINTF("NETWORK_ICON hash mismatch!\n");
        return APDU_RESPONSE_INVALID_DATA;
    }

    DYNAMIC_NETWORK_INFO[g_icon_payload.slot].icon.bitmap =
        (const uint8_t *) g_network_icon[g_icon_payload.slot].bitmap;
    DYNAMIC_NETWORK_INFO[g_icon_payload.slot].icon.width = U2LE(data, 0);
    DYNAMIC_NETWORK_INFO[g_icon_payload.slot].icon.height = U2LE(data, 2);
    // BPP is stored in the upper 4 bits of the 5th byte
    DYNAMIC_NETWORK_INFO[g_icon_payload.slot].icon.bpp = data[4] >> 4;
    DYNAMIC_NETWORK_INFO[g_icon_payload.slot].icon.isFile = true;
    COPY_FIELD(DYNAMIC_NETWORK_INFO[g_icon_payload.slot].icon.bitmap);
    print_icon_info();
    return APDU_RESPONSE_OK;
}
...
static uint16_t handle_first_icon_chunk(const uint8_t *data, uint8_t length) {
    ...
    explicit_bzero(g_network_icon[g_icon_payload.slot].bitmap, MAX_ICON_LEN);
...
static uint16_t handle_next_icon_chunk(const uint8_t *data, uint8_t length) {
    ...
    // Feed into payload
    memcpy(g_network_icon[g_icon_payload.slot].bitmap + g_icon_payload.received_size, data, length);
 ...
 * @return APDU Response code
 */
static uint16_t handle_icon_chunks(uint8_t p2, const uint8_t *data, uint8_t length) {
    uint16_t sw = APDU_RESPONSE_UNKNOWN;
    uint16_t slot = 0;

    // Check the received chunk index
    if ((p2 & 0x0F) == P2_FIRST_CHUNK) {
        slot = (p2 >> 4) & 0x0F;
        if (slot >= MAX_DYNAMIC_NETWORKS) {
            PRINTF("Error: Invalid Slot nb (%u)!\n", slot);
            return APDU_RESPONSE_INVALID_P1_P2;
        }
        // Init the dynamic network feature with the 1st chunk
        sw = handle_first_icon_chunk(data, length);
        if (sw != APDU_RESPONSE_OK) {
            return sw;
        }
        g_icon_payload.slot = slot;
    } else if (p2 != P2_NEXT_CHUNK) {
...
static bool verify_signature(s_sig_ctx *sig_ctx) {
    ...
    CX_CHECK(check_signature_with_pubkey("Dynamic Network",
                                         hash,
                                         sizeof(hash),
                                         DYNAMIC_NETWORK_PUB_KEY,
                                         sizeof(DYNAMIC_NETWORK_PUB_KEY),
#ifdef HAVE_LEDGER_PKI
 ...
 * @param[in] slot Network slot index
 */
static void print_network_info(uint8_t slot) {
    char chain_str[sizeof(uint64_t) * 2 + 1] = {0};

    PRINTF("****************************************************************************\n");
    u64_to_string(DYNAMIC_NETWORK_INFO[slot].chain_id, chain_str, sizeof(chain_str));
    PRINTF("[NETWORK] - Registered in slot %d: %s (%s), for chain_id %s\n",
           slot,
           DYNAMIC_NETWORK_INFO[slot].name,
           DYNAMIC_NETWORK_INFO[slot].ticker,
           chain_str);
}
 ...
 * @return APDU Response code
 */
static uint16_t parse_tlv(const uint8_t *data, uint8_t length, uint8_t slot) {
    uint16_t offset = 0;
    ...
    // Reset the structures
    explicit_bzero(&DYNAMIC_NETWORK_INFO[slot], sizeof(network_info_t));
#ifdef HAVE_NBGL
    explicit_bzero(&g_network_icon[slot], sizeof(network_icon_t));
#endif
    ...
    while (offset != length) {
        ...
        switch (field_tag) {
            ...
            case TAG_BLOCKCHAIN_FAMILY:
                ...
                break;
            case TAG_CHAIN_ID:
                sw = parse_chain_id(data + offset, field_len, slot);
                break;
            case TAG_NETWORK_NAME:
                sw = parse_name(data + offset, field_len, slot);
                break;
            case TAG_TICKER:
                sw = parse_ticker(data + offset, field_len, slot);
                break;
            case TAG_NETWORK_ICON_HASH:
#ifdef HAVE_NBGL
                sw = parse_icon_hash(data + offset, field_len, slot);
#endif
                break;
...
uint16_t handleNetworkConfiguration(uint8_t p1,
    ...
    switch (p1) {
        case P1_NETWORK_CONFIG:
            if (p2 >= MAX_DYNAMIC_NETWORKS) {
                PRINTF("Error: Invalid Slot nb (%u)!\n", p2);
                sw = APDU_RESPONSE_INVALID_P1_P2;
                break;
            }
            sw = parse_tlv(data, length, p2);
            break;

        case P1_NETWORK_ICON:
#ifdef HAVE_NBGL
            sw = handle_icon_chunks(p2, data, length);
#else
