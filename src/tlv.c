...
static bool parse_der_value(const uint8_t *payload, uint16_t size, uint16_t *offset, uint32_t *value) {
    uint8_t byte_length;
 ...
 * @param[in,out] context structure passed to the handler function so the context from one handler invocation to the next can be kept
 * @return whether the parsing (and handling) was successful or not
 */
bool parse_tlv(const uint8_t *payload, uint16_t size, f_tlv_data_handler handler, void *context) {
    ...
    while (offset < size) {
        switch (state) {
            case TLV_STATE_TAG:
                ...
                if (!parse_der_value(payload, size, &offset, &tmp_value) || (tmp_value > UINT8_MAX)) {
                    return false;
                }
            ...
            case TLV_STATE_LENGTH:
                if (!parse_der_value(payload, size, &offset, &tmp_value) || (tmp_value > UINT16_MAX)) {
                    return false;
                }
