...
static bool handle_array(const s_tlv_data *data, s_data_path_context *context) {
    s_path_array_context ctx;

    ctx.args = &context->data_path->elements[context->data_path->size].array;
    if (!tlv_parse(data->value, data->length, (f_tlv_data_handler)&handle_array_struct, &ctx)) return false;
...
static bool handle_slice(const s_tlv_data *data, s_data_path_context *context) {
    s_path_slice_context ctx;

    ctx.args = &context->data_path->elements[context->data_path->size].slice;
    if (!tlv_parse(data->value, data->length, (f_tlv_data_handler)&handle_slice_struct, &ctx)) return false;
...
static bool path_tuple(const s_tuple_args *tuple, uint32_t *offset, uint32_t *ref_offset) {
    *ref_offset = *offset;
    *offset += (tuple->value * PARAMETER_LENGTH);
    return true;
}

static bool path_ref(const s_calldata *calldata, uint32_t *offset, uint32_t *ref_offset) {
    if (!U4BE_from_parameter(&calldata->ptr[*offset], offset)) {
        return false;
    }
    *offset += *ref_offset;
    return true;
...
static bool path_leaf(const s_leaf_args *leaf,
    ...
    switch (leaf->type) {
        case LEAF_TYPE_STATIC:
            collection->value[collection->size].length = PARAMETER_LENGTH;
            break;

        case LEAF_TYPE_DYNAMIC:
            if (!U4BE_from_parameter(&calldata->ptr[*offset],
                                     &collection->value[collection->size].length)) {
                return false;
            }
    ...
    collection->value[collection->size].ptr = &calldata->ptr[*offset];
    collection->size += 1;
    return true;
}
...
static bool path_slice(const s_slice_args *slice, s_parsed_value_collection *collection) {
    uint32_t start;
    uint32_t end;
    uint32_t value_length = collection->value[collection->size - 1].length;
...
static bool path_array(const s_array_args *array,
    ...
    if (!U4BE_from_parameter(&calldata->ptr[*offset], &array_size)) {
        return false;
    }
    *offset += PARAMETER_LENGTH;
    if (arrays_info->index == arrays_info->depth) {
        // new depth
        if (array->subset) {
            arrays_info->passes_remaining[arrays_info->index] = array->length;
        } else {
            arrays_info->passes_remaining[arrays_info->index] = array_size;
        }
        arrays_info->depth += 1;
    }
    if (array->subset) {
        idx = (array->start < 0) ? ((int16_t)array_size + array->start)
                                 : array->start;
        idx += (array->length - arrays_info->passes_remaining[arrays_info->index]);
    } else {
        idx = (array_size - arrays_info->passes_remaining[arrays_info->index]);
    }
    *ref_offset = *offset;
    *offset += (idx * array->weight * PARAMETER_LENGTH);
    arrays_info->index += 1;
    return true;
}
...
bool data_path_get(const s_data_path *data_path, const s_calldata *calldata, s_parsed_value_collection *collection) {
    bool ret;
    uint32_t offset;
    uint32_t ref_offset;
    ...
    do {
        arinf.index = 0;
        offset = 4; // skip selector
        ref_offset = offset;
        for (int i = 0; i < data_path->size; ++i) {
            if ((offset + PARAMETER_LENGTH) > calldata->size) {
                return false;
            }
            ...
            switch (data_path->elements[i].type) {
                ...
                case ELEMENT_TYPE_ARRAY:
                    ret = path_array(&data_path->elements[i].array,
                                     &offset,
                                     &ref_offset,
                                     calldata,
                                     &arinf);
                ...
                case ELEMENT_TYPE_REF:
                    ret = path_ref(calldata, &offset, &ref_offset);
                    break;
