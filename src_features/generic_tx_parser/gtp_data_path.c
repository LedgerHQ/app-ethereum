...
static bool handle_array(const s_tlv_data *data, s_data_path_context *context) {
    ...
    if (!parse_tlv(data->value, data->length, (f_tlv_data_handler)&handle_array_struct, &ctx)) return false;
...
static bool handle_slice(const s_tlv_data *data, s_data_path_context *context) {
    ...
    if (!parse_tlv(data->value, data->length, (f_tlv_data_handler)&handle_slice_struct, &ctx)) return false;
