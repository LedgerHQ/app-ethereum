...
static bool handle_name(const s_tlv_data *data, s_field_ctx *context) {
    if (data->length >= sizeof(context->field->name)) {
        return false;
    }
    memcpy(context->field->name, data->value, data->length);
    context->field->name[data->length] = '\0';
    return true;
}
...
static bool handle_param(const s_tlv_data *data, s_field_ctx *context) {
    ...
    return parse_tlv(data->value, data->length, handler, &param_ctx);
}
