    ...
    if ((ret = value_get(&param->calldata, &calldatas))) {
        if ((ret = value_get(&param->contract_addr, &contract_addrs)) && (contract_addrs.size == calldatas.size)) {
            if ((!param->has_chain_id || ((ret = value_get(&param->chain_id, &chain_ids)) && (chain_ids.size == calldatas.size))) &&
                (!param->has_selector || ((ret = value_get(&param->selector, &selectors)) && (selectors.size == calldatas.size))) &&
                (!param->has_amount || ((ret = value_get(&param->amount, &amounts)) && (amounts.size == calldatas.size))) &&
                (!param->has_spender || ((ret = value_get(&param->spender, &spenders)) && (spenders.size == calldatas.size)))) {
                for (int i = 0; i < calldatas.size; ++i) {
                    if (calldatas.value[i].length > 0) {
                        ...
                        // populate tx context
                        set_tx_ctx_to(contract_addr);
                        if (param->has_amount) {
                            uint8_t amount[INT256_LENGTH];
                            buf_shrink_expand(amounts.value[i].ptr, amounts.value[i].length, amount, sizeof(amount));
                            set_tx_ctx_amount(amount);
                        }
                        if (param->has_spender) {
                            uint8_t from[ADDRESS_LENGTH];
                            buf_shrink_expand(spenders.value[i].ptr, spenders.value[i].length, from, sizeof(from));
