...
static bool handle_value(const s_tlv_data *data, s_param_raw_context *context) {
    ...
    return parse_tlv(data->value, data->length, (f_tlv_data_handler)&handle_value_struct, &ctx);
}
...
bool format_int(const s_value *def, const s_parsed_value *value, char *buf, size_t buf_size) {
    ...
    switch (def->type_size * 8) {
        case 256:
            convertUint256BE(value->ptr, value->length, &uv.value256);
            ret = tostring256_signed(&uv.value256, 10, buf, buf_size);
            break;
        case 128:
            convertUint128BE(value->ptr, value->length, &uv.value128);
            ret = tostring128_signed(&uv.value128, 10, buf, buf_size);
            break;
        case 64:
            uv.value64 = read_u64_be(value->ptr, 0);
            ret = snprintf(buf, buf_size, "%" PRId64, uv.value64) > 0;
            break;
        case 32:
            uv.value32 = read_u32_be(value->ptr, 0);
            ret = snprintf(buf, buf_size, "%" PRId32, uv.value32) > 0;
            break;
        case 16:
            uv.value16 = read_u16_be(value->ptr, 0);
...
static bool format_addr(const s_value *def, const s_parsed_value *value, char *buf, size_t buf_size) {
    (void)def;
    return getEthDisplayableAddress((uint8_t*)value->ptr,
                                    buf,
                                    buf_size,
                                    chainConfig->chainId);
}

static bool format_bool(const s_value *def, const s_parsed_value *value, char *buf, size_t buf_size) {
    (void)def;
    snprintf(buf, buf_size, "%s", value->ptr[0] ? "true" : "false");
    return true;
}

static bool format_bytes(const s_value *def, const s_parsed_value *value, char *buf, size_t buf_size) {
    (void)def;
    snprintf(buf, buf_size, "0x%.*h", value->length, value->ptr);
    if ((2 + (value->length * 2) + 1) > buf_size) {
        strcpy(&buf[buf_size - 1 - 3], "...");
    }
    return true;
}

static bool format_string(const s_value *def, const s_parsed_value *value, char *buf, size_t buf_size) {
    (void)def;
    memcpy(buf, value->ptr, MIN(buf_size, value->length));
