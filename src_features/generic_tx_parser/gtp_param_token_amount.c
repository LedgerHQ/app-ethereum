...
static bool handle_native_currency(const s_tlv_data *data, s_param_token_amount_context *context) {
    ...
    memcpy(&context->param->native_addrs[context->param->native_addr_count][ADDRESS_LENGTH - data->length],
           data->value,
           data->length);
...
static bool handle_above_threshold_msg(const s_tlv_data *data, s_param_token_amount_context *context) {
...
bool handle_param_token_amount_struct(const s_tlv_data *data, s_param_token_amount_context *context) {
    bool ret;
...
bool format_param_token_amount(const s_param_token_amount *param, const char *name) {
    ...
    for (int i = 0; i < collec_value.size; ++i) {
        if (param->has_token) {
            if (match_native(collec_token.value[i].ptr, param)) {
                ticker = get_displayable_ticker(&chain_id, chainConfig);
                decimals = WEI_TO_ETHER;
            } else {
                const tokenDefinition_t *token;
                if ((token = (const tokenDefinition_t*)get_asset_info_by_addr(collec_token.value[i].ptr)) != NULL) {
                    ticker = token->ticker;
