...
bool handle_param_trusted_name_struct(const s_tlv_data *data, s_param_trusted_name_context *context) {
    bool ret;
...
bool format_param_trusted_name(const s_param_trusted_name *param, const char *name) {
    ...
    for (int i = 0; i < values.size; ++i) {
        buf_shrink_expand(values.value[i].ptr,
        ...
        if ((tname = get_trusted_name(param->type_count,
            ...
            getEthDisplayableAddress((uint8_t*)values.value[i].ptr,
                                     buf,
                                     buf_size,
                                     chainConfig->chainId);
