...
static uint16_t handle_first_sign_chunk(const uint8_t *payload, uint8_t length, uint8_t *offset, e_sign_mode mode) {
    ...
    *offset += (length - length_tmp);
    ...
    if (init_tx(&txContext, &global_sha3, &tmpContent.txContent, mode == SIGN_MODE_STORE) == false) {
        return APDU_RESPONSE_INVALID_DATA;
    }
    if (*offset >= length) {
        PRINTF("Invalid data\n");
    ...
    if (tx_type <= MAX_TX_TYPE) {
        ...
        if (cx_hash_no_throw((cx_hash_t *) &global_sha3, 0, &tx_type, sizeof(tx_type), NULL, 0) != CX_OK) {
            return APDU_RESPONSE_INVALID_DATA;
        }
...
uint16_t handleSign(uint8_t p1,
    ...
    switch (p2) {
        ...
        case SIGN_MODE_STORE:
            ...
            switch (p1) {
                case P1_FIRST:
                    if ((sw = handle_first_sign_chunk(payload, length, &offset, p2)) != APDU_NO_RESPONSE) {
                        return sw;
                    }
                    break;
                case P1_MORE:
