...
static char *_strdup(const char *src) {
    ...
    if ((dst = mem_alloc(strlen(src) + 1)) != NULL) {
        strcpy(dst, src);
    }
    return dst;
}
...
static bool prepare_infos(nbgl_contentInfoList_t *infos) {
    ...
    if (((keys = mem_alloc_and_align(sizeof(*keys) * MAX_INFO_COUNT, __alignof__(*keys))) == NULL) ||
        ((values = mem_alloc_and_align(sizeof(*values) * MAX_INFO_COUNT, __alignof__(*values))) == NULL)) {
        return false;
    }
    ...
    if ((extensions = mem_alloc_and_align(sizeof(*extensions) * count, __alignof__(*extensions))) == NULL) {
        return false;
    }
    explicit_bzero(extensions, sizeof(*extensions) * count);

    if (contract_idx != -1) {
        addr = get_contract_addr();
        ...
        if (get_tx_chain_id() == ETHEREUM_MAINNET_CHAINID) {
            if ((extensions[contract_idx].explanation = _strdup("Scan to view on Etherscan")) == NULL) {
                return false;
            }
            snprintf(tmp_buf, tmp_buf_size, "https://etherscan.io/address/%s", extensions[contract_idx].title);
...
bool ui_gcs(void) {
    ...
    if (!prepare_infos(&infos)) {
        return false;
    }
    nbgl_tipBox_t tip_box = {
        .icon = &C_review_info_button,
        .text = NULL,
        .modalTitle = "Contract information",
        .infos.nbInfos = infos.nbInfos,
        .infos.infoTypes = infos.infoTypes,
        .infos.infoContents = infos.infoContents,
        .infos.infoExtensions = infos.infoExtensions,
        .infos.withExtensions = infos.withExtensions,
        .type = INFOS_LIST
    };
    ...
    pair_list.nbPairs = field_table_size() + 1;
    show_network = get_tx_chain_id() != chainConfig->chainId;
    if (show_network) {
        pair_list.nbPairs += 1;
    }
    if ((pairs = mem_alloc_and_align(sizeof(*pairs) * pair_list.nbPairs, __alignof__(*pairs))) == NULL) {
        return false;
    }
    explicit_bzero(pairs, sizeof(*pairs) * pair_list.nbPairs);
    ...
    if (show_network) {
        pairs[pair_list.nbPairs - 2].item = _strdup("Network");
        if (get_network_as_string(tmp_buf, tmp_buf_size) != APDU_RESPONSE_OK) {
            return false;
        }
        pairs[pair_list.nbPairs - 2].value = _strdup(tmp_buf);
    }

    pairs[pair_list.nbPairs - 1].item = _strdup("Max fees");
    ...
    pairs[pair_list.nbPairs - 1].value = _strdup(tmp_buf);
