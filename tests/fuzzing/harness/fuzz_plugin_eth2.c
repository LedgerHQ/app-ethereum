/**
 * Fuzzing harness for the ETH2 internal plugin (deposit operations)
 *
 * Note: ETH2 plugin is guarded by HAVE_ETH2 define.
 * This harness will only be effective when HAVE_ETH2 is defined in the build.
 */
...
static int fuzz_eth2_plugin(const uint8_t *data, size_t size) {
    ...
    eth2_plugin_call(ETH_PLUGIN_INIT_CONTRACT, &init_contract);
    ...
    uint32_t param_offset = SELECTOR_SIZE;  // Starts at 4 (after selector)
    while (size - offset >= PARAMETER_LENGTH) {
        ...
        eth2_plugin_call(ETH_PLUGIN_PROVIDE_PARAMETER, &provide_param);
    ...
    eth2_plugin_call(ETH_PLUGIN_FINALIZE, &finalize);
    if (finalize.result != ETH_PLUGIN_RESULT_OK && finalize.result != ETH_PLUGIN_RESULT_FALLBACK) {
        return 0;
    }

    // If fallback, skip the rest of the flow
    ...
    if (finalize.tokenLookup1 || finalize.tokenLookup2) {
        ...
        eth2_plugin_call(ETH_PLUGIN_PROVIDE_INFO, &provide_info);
    ...
    eth2_plugin_call(ETH_PLUGIN_QUERY_CONTRACT_ID, &query_id);
    ...
    for (uint8_t i = 0; i < total_screens; i++) {
        ...
        eth2_plugin_call(ETH_PLUGIN_QUERY_CONTRACT_UI, &query_ui);
