...
def prepare_network_information(name: str,
    ...
    chunks.append(serialize(P1Type.NETWORK_CONFIG, 0x00, payload))

    if icon:
        # Serialize the icon
        p2 = P2Type.FIRST_CHUNK
        while len(icon) > 0:
            chunks.append(serialize(P1Type.NETWORK_ICON, p2, icon[:0xff]))
            icon = icon[0xff:]
            p2 = P2Type.NEXT_CHUNK
    return chunks
